...
# Utilities import
from castor_etc.utils.conversions import (
    calc_photon_energy,
...
from castor_etc.utils.image_processing import gen_unconv_image
...
class Observation:
    ...
    def __init__(self, TelescopeObj, SourceObj, BackgroundObj, stellar_model_dir):
        ...
        if not isinstance(SourceObj, PointSource):
            raise TypeError(
                "SourceObj must be a `castor_etc.PointSource` object. Other sources are not currently supported"
            )
        ...
        # NOTE: this requires additional install of an external, pretty large stellar model
        # This should follow the same design philosophy as the overall package:
        #   FOR CANFAR USERS: it will search CASTOR folders on CANFAR
        #   FOR LOCAL USERS: we should provide a way to easily install these files, and/or define a suitable "entrypoint" for them
        ...
        self.tend = 6.0 / 24.0 * u.d  # light curve end time [days]

        self.ccd_dim = [
            self.TelescopeObj.transit_ccd_dim[0],
            self.TelescopeObj.transit_ccd_dim[1],
        ]  # [pxl,pxl] used for plotting (including converting ra,dec -> x_pxl, y_pxl)
        ...
        self.noise_sources = [
            ...
            "jitter",
            "sky_background",
        ]  # List of noise sources to include
        ...
        self.detector_aperture = 1.0 * u.m
        self.detector_pixscale = 10.0 * u.um
        ...
        self.ccd_aperture = (
            _OPTIMAL_APER_FACTOR * 0.5 * TelescopeObj.fwhm.to(u.arcsec).value
        ) / TelescopeObj.px_scale.to(
            u.arcsec
        ).value  # radius [pxl] for aperture photometry
    ...
    def _photon_count(
        ...
        wl, fl = getStarData(
            ...
            metallicity,
            logg,
            model_grid=self.SourceObj.stellar_model_grid,
            stellar_model_dir=self.stellar_model_dir,
        )  # Angstrom, flam

        wavelength_AA = wl.to(u.AA).value

        # Estimate distance using Gmag and estimated Gabs
        dpc = 10 ** ((Gmag - Gmag_abs) / 5.0 + 1)
        ...
        tr_fcn = np.loadtxt(
            join(
                DATAPATH,
                "transit_data/instrument_data/transmission_functions/",
                "GaiaDR2_Passbands.dat",
            )
        )
        ...
        for j in [
            ...
            3,
            5,
        ]:  # Select columns listing transmission values, not the errors
            ti = np.where(tr_fcn[:, j] < 99)[0]
            _int_tr = np.interp(
                wavelength_AA, tr_fcn[ti, 0], tr_fcn[ti, j], left=0, right=0
            )
            gaia_flux.append(np.trapz(_int_tr * fl, wavelength_AA))

        # Interpolate passband to spectrum resolution
        passband_wavelengths = (
            self.TelescopeObj.full_passband_curves[self.passband_name]["wavelength"]
            .to(u.AA)
            .value
        )
        ...
        e_rate = convert_electron_flux_mag(
            var1=ab_mag,
            var1_type="mag",
            var2_type="electron",
            phot_zpt=self.TelescopeObj.phot_zpts[self.passband_name],
        )[0]
    ...
    def specify_bandpass(self, passband_name=None):
        ...
        if passband_name not in self.TelescopeObj.passbands:
            raise TypeError(
                "Specified bandpass must belong to CASTOR telescope filter, i.e., ['uv','u','g']"
            )
    ...
    def id_guide_stars(self, gs_criteria=None, plot_SN=False):
        ...
        if gs_criteria != None:
            if isinstance(gs_criteria["exptime"], u.Quantity):
                try:
                    ...
                    raise TypeError(
                        " guide start exposure time must be a `astropy.Quantity` time (e.g., sec,day) "
                    )
        ...
        _nstack, _exptime, _quiet = (
            self.nstack,
            self.exptime.to(u.second),
            self.quiet,
        )  # Store previous nstack and exptime
        self.nstack, self.exptime, self.quiet = (
            1,
            self.gs_criteria["exptime"].to(u.second),
            True,
        )
        SN_pix, _SN_ap = self.scene_sim(
            return_SN_only=True
        )  # Get guide star S/N per pixel
        ...
        if len(self.SourceObj.gaia["ra"] > 1):
            ...
            _ti = np.where(
                (self.SourceObj.gaia["x"][gs_i] > self.gs_criteria["ccd_aperture"])
                & (
                    self.SourceObj.gaia["x"][gs_i]
                    < (self.ccd_dim[0] - self.gs_criteria["ccd_aperture"])
                )
                & (self.SourceObj.gaia["y"][gs_i] > self.gs_criteria["ccd_aperture"])
                & (
                    self.SourceObj.gaia["y"][gs_i]
                    < (self.ccd_dim[1] - self.gs_criteria["ccd_aperture"])
                )
            )[0]
            gs_i = gs_i[_ti]
            # Get maximum S/N associated with each potential guide star
            _xx, _yy = np.meshgrid(
                np.arange(self.ccd_dim[0]), np.arange(self.ccd_dim[1])
            )
            _xx, _yy = _xx.T, _yy.T
            ...
            for i in range(len(gs_i)):
                ti = (
                    np.sqrt(
                        (_xx - self.SourceObj.gaia["x"][gs_i[i]]) ** 2
                        + (_yy - self.SourceObj.gaia["y"][gs_i[i]]) ** 2
                    )
                    < self.gs_criteria["ccd_aperture"]
                )
                ...
                for i in range(len(gs_i)):
                    if (i in ki) == False:
                        _sep = np.sqrt(
                            (
                                self.SourceObj.gaia["x"][gs_i]
                                - self.SourceObj.gaia["x"][gs_i[i]]
                            )
                            ** 2
                            + (
                                self.SourceObj.gaia["y"][gs_i]
                                - self.SourceObj.gaia["y"][gs_i[i]]
                            )
                            ** 2
                        )
        ...
        if plot_SN:
            ...
            ax.scatter(
                self.SourceObj.gaia["Gmag"][self.gaia["gs_i"]],
                self.gaia["gs_SN_pix_max"],
            )
    ...
    def _calc_source_weights(self, center):
        ...
        """
        telescope_standard_dev_sq = (self.TelescopeObj.fwhm.to(u.arcsec).value ** 2) / (
            4 * 2 * np.log(2)
        )
    ...
    def _point_source_sim(self, target_flux_fraction, scene_phot_count):
        ...
        for istack in range(self.nstack):
            ...
            if "illum" in self.noise_sources:
                il = np.genfromtxt(
                    fname=join(
                        DATAPATH, "transit_data/instrument_data", self.illumfile
                    ),
                    delimiter=",",
                )
            ...
            if "shot" in self.noise_sources:
                source_count_noise += np.sqrt(np.abs(source_count)) * np.random.normal(
                    size=(source_count.shape[0], source_count.shape[1])
                )

            if (
                ("read" in self.noise_sources)
                | ("dark" in self.noise_sources)
                | ("sky_background" in self.noise_sources)
            ):
                _std_dev = 0.0
                if "read" in self.noise_sources:
                    _std_dev += (
                        self.read_noise * self.exptime.to(u.second).value / self.gain
                    )
                if "dark" in self.noise_sources:
                    _std_dev += (
                        self.dark_noise * self.exptime.to(u.second).value / self.gain
                    )
                if "sky_background" in self.noise_sources:
                    _std_dev += (
                        self.sky_background_noise[self.passband_name]
                        * self.exptime.to(u.second).value
                        / self.gain
                    )

                source_count_noise += _std_dev * np.random.normal(
                    size=(source_count_noise.shape[0], source_count_noise.shape[1])
                )
    ...
    def scene_sim(
        ...
        for istack in range(self.nstack):
            ...
            if (istack == 0) | ((xjit > 1.0e-3) | (yjit > 1.0e-3)):
                for isource in range(nsrc):
                    ...
                    pixels1 = gen_unconv_image(
                        self, starmodel_flux[isource], xcoo, ycoo
                    )
                    if isource == 0:
            ...
            xpad = self.xpad * self.noversample
            ypad = self.ypad * self.noversample
            pixels_conv_ras = pixels_conv[
                ypad : pshape[0] - ypad, xpad : pshape[1] - xpad
            ]

            # Scale to native resolution (remove oversampling)
            pixels_conv_ras_nav = downscale_local_mean(
                pixels_conv_ras, (self.noversample, self.noversample)
            )  # array of e/pix
            ...
            SN_pix[ti] = pixels_conv_ras_nav[ti] / np.sqrt(
                pixels_conv_ras_nav[ti]
                + np.sqrt(self.nstack) * self.read_noise
                + (self.dark_noise + self.sky_background_noise[self.passband_name])
                * self.nstack
                * self.exptime.to(u.second).value
            )
            ...
            if istack == 0:
                ...
                aper = EllipticalAperture(
                    positions=_coord, a=aper_radius_px, b=aper_radius_px, theta=0
                )

                _ap_phot = aperture_photometry(
                    np.abs(pixels_conv_ras_nav), aper, method="center"
                )
            ...
            if "shot" in self.noise_sources:
                # shot-noise
                pixels_conv_ras_nav_noise += np.sqrt(
                    np.abs(pixels_conv_ras_nav)
                ) * np.random.normal(
                    size=(pixels_conv_ras_nav.shape[0], pixels_conv_ras_nav.shape[1])
                )
            ...
            if (
                ...
                pixels_conv_ras_nav_noise += _std_dev * np.random.normal(
                    size=(
                        pixels_conv_ras_nav_noise.shape[0],
    ...
    def plot_fov(self, plot_guide_stars=True, vmin=None, vmax=None, add_scene_sim=True):
        """
        Ask James
        ...
        Gmag_lim = [
            np.min(self.SourceObj.gaia["Gmag"]),
            np.max(self.SourceObj.gaia["Gmag"]),
        ]
        sym_lim = [0.5, 100]
        if Gmag_lim[1] == Gmag_lim[0]:
            ...
            sym_size = (
                (Gmag_lim[1] - self.SourceObj.gaia["Gmag"])
                / (Gmag_lim[1] - Gmag_lim[0])
            ) * (sym_lim[1] - sym_lim[0]) + sym_lim[0]

        # Plot target
        ax.scatter(
            self.SourceObj.gaia["x"][0],
            self.SourceObj.gaia["y"][0],
            edgecolor="r",
            marker="o",
            s=80,
            facecolor="None",
        ...
        if plot_guide_stars:
            ...
            if "gs_i" in self.gaia.keys():
                ax.scatter(
                    self.SourceObj.gaia["x"][self.gaia["gs_i"]],
    ...
    def specify_pl_model(self, RpRs, P, t0, b, aRs):
        ...
        if not isinstance(RpRs, list):
            raise TypeError(
                "The radius ratio can either be a scalar, 1D vector, or a 2D array"
            )
        ...
        pl_model = {"RpRs": RpRs, "P": P, "t0": t0, "b": b, "aRs": aRs}
        self.pl_model = pl_model

    def specify_exposure_parameters(
        self, exptime=60 * u.second, nstack=10, tstart=0.0 * u.d, tend=6.0 / 24.0 * u.d
    ):
        """
        ...
        if isinstance(exptime, u.Quantity):
            try:
                ...
                raise TypeError(
                    "exptime must be a `astropy.Quantity` time (e.g., sec, day)"
                )
        ...
        if isinstance(tstart, u.Quantity):
            try:
                ...
                raise TypeError(
                    "tstart must be a `astropy.Quantity` time (e.g., sec, day)"
                )
        if isinstance(tend, u.Quantity):
            try:
                ...
                raise TypeError(
                    "tend must be a `astropy.Quantity` time (e.g., sec,day)"
                )
    ...
    def calc_pl_model(self, model="pytransit_QuadraticModel", t_grid=[], exp_time=-1):
        ...
        if len(t_grid) == 0:
            ...
            t_grid = np.arange(
                self.tstart.to(u.d).value,
                self.tend.to(u.d).value,
                cadence / (24.0 * 3600.0),
            )
        nt = len(t_grid)
        ...
        if hasattr(self, "pl_model"):
            ...
            if model == "pytransit_QuadraticModel":
                ...
                if exp_time < 0:
                    tot_exptime = (
                        self.nstack * self.exptime.to(u.second).value / (24.0 * 3600.0)
                    )
                else:
                    tot_exptime = exp_time / (24.0 * 3600.0)
                ...
                for n in range(n_pl):
                    ...
                    if "aRs" in self.pl_model.keys():
                        ...
                        aRs = (
                            self.pl_model["rho_s"]
                            * const_G
                            * (self.pl_model["P"][n] * 24.0 * 3600.0) ** 2
                            / (3.0 * np.pi)
                        ) ** (1.0 / 3.0)
                    ...
                    _pl_lc.append(
                        tm.evaluate_ps(
                            _pv[0], ldc, _pv[1], _pv[2], _pv[3], _pv[4], _pv[5], _pv[6]
                        )
                        - 1.0
                    )
    ...
    def lc_sim(self, quiet=False, return_lc=False, cadence=-1, exp_time=-1):
        ...
        pl_lc = self.calc_pl_model(t_grid=t_grid, exp_time=exp_time) + 1.0
        ...
        for n in range(nt):
            obs, err = self._point_source_sim(
                target_flux_fraction=pl_lc[n], scene_phot_count=scene_phot_count
            )
            i = 0
    ...
    def plot_lc(self, plot_model=True, exp_time=-1, t_from_mid=True, t_unit="d"):
        """
        ...
        t_offset = 0.0
        if t_from_mid:
            t_offset = -(self.lc_t[-1] - self.lc_t[0]) / 2.0
        ...
        ax.scatter(
            (self.lc_t + t_offset) * t_scale,
            (self.lc_fl[:, 0] + y_offset) * y_scale,
            label="Simulated CASTOR",
        )
        ax.errorbar(
            (self.lc_t + t_offset) * t_scale,
            (self.lc_fl[:, 0] + y_offset) * y_scale,
            self.lc_err[:, 0] * y_scale,
            ls="None",
        )
        if t_from_mid:
        ...
        if plot_model & hasattr(self, "pl_model"):
            ...
            pl_lc = self.calc_pl_model(t_grid=_t_grid, exp_time=exp_time) + 1.0
            ax.plot(
                (_t_grid + t_offset) * t_scale,
                (pl_lc + y_offset) * y_scale,
                c="k",
                label="Transit Model",
            )
